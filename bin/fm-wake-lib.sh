...
fm_watcher_supervision_verdict() {
  ...
  FM_WATCHER_VERDICT_REASON=stale-beacon
  beat="$state/.last-watcher-beat"
  # Awake-time age: beacon staleness across a system sleep window is not a
  # supervision lapse - the writer had no CPU while asleep (see
  # fm_path_age_since_system_wake). Every model's freshness read here must
  # ignore slept time or the clamshell false-death class returns.
  age=$(fm_path_age_since_system_wake "$beat")
  case "$age" in
    ''|*[!0-9]*) ;;
