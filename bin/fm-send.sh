...
#   An acknowledged order rides the INBOX plane like any other text steer to a
#   recorded task: --ack's own eligibility rules (a task in this home, not a
#   secondmate, not a parser-native invocation) are exactly the conditions that
#   select that plane, so the mark layers an order-level acknowledgment over
#   durable delivery rather than adding a second transport. A proven delivery
#   failure discards the record; a durable enqueue keeps it armed, because the
#   record IS the delivery and detection must keep tracking the order until the
#   worker acknowledges it. Plain sends without --ack are unchanged.
#
# Two data planes:
#
...
TARGET_TASK_ID=
# Armed by the --ack path below; declared here so the single known-undelivered
# cleanup owner can clear the record from every proven-failure exit.
ACK_TOKEN=
ACK_TASK_ID=
# The one owner of "drop the pending-ack record this invocation armed". Every
# exit that proves the order was not delivered calls it, including the inbox
# plane's pre-transport validation failures: an armed record left behind would
# escalate an order the worker was never given.
fm_send_discard_armed_ack() {
  [ -n "$ACK_TOKEN" ] || return 0
  fm_steer_ack_discard "$STATE" "$ACK_TASK_ID" "$ACK_TOKEN"
}
# The one owner of "this send is known undelivered" once transport was actually
# attempted: the pending-reply expectation and any armed pending-ack record go
# together, because both track an order that never landed.
fm_send_known_undelivered_cleanup() {
  local rc=0
  fm_send_discard_armed_ack || rc=1
  [ -n "$PENDING_REPLY_CORR" ] || return "$rc"
  if [ "$PENDING_REPLY_CREATED" = 1 ]; then
    fm_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || rc=1
  else
    fm_pending_reply_reset_known_undelivered "$STATE" "$PENDING_REPLY_CORR" || rc=1
  fi
  return "$rc"
}
...
if [ "${1:-}" = "--key" ]; then
  ...
  # the durable pending-ack record before delivery. The eligibility rules below
  # are exactly the conditions that select the inbox plane, so an armed record
  # always accompanies a durable inbox record rather than a typed submit. It is
  # discarded only where delivery is proven to have failed, through the single
  # known-undelivered cleanup owner (bin/fm-steer-ack-lib.sh owns the record).
  if [ "$WANT_ACK" = 1 ]; then
    if [ -z "$TARGET_SELECTOR" ] || [ -z "$TARGET_META" ]; then
