...
# is required. Beacon freshness is judged on awake time, never counting system
# sleep windows (fm_path_age_since_system_wake in bin/fm-wake-lib.sh), so a
# clamshell sleep never reads as a supervision lapse. The banner names the true
# failing condition (a missing live
# watcher process vs a genuinely stale beacon), and when the watcher singleton
# lock still names a dead holder pid - the beacon having outlived its writer -
# the banner names that dead holder too. The full banner is emitted once
