...
#   is detected (a local-only project), the clean pooled worktree instead
...
#   An unreachable detected origin, a default branch unresolvable on the remote
#   or on the primary local copy, an unreadable primary copy, an unverifiable
#   fetched tip, or a non-clean worktree refuses a fresh spawn rather than
...
# A pool clone with detected `origin` configuration resolves that base over the
# network exactly as before; spawn_worktree_has_origin_config owns the
# detection, covering Git's effective include/includeIf chain rather than only
# a resolvable url. A local-only project has no `origin` in its pool clone
...
# Both paths share one leading cleanliness refusal and converge on one hard
# reset and post-reset HEAD verification, so no path can discard uncommitted
# work or launch from an unverified base.
