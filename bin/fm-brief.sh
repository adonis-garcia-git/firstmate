...
# Ship and scout scaffolds additionally teach the acknowledgment reply for a
# token-marked [ack=<token>] order sent via fm-send --ack: that mark rides an
# ordinary inbox record, and the status-line reply is the order-level ack
# layered over it (bin/fm-steer-ack-lib.sh owns that record and escalation
# contract).
...
4. Report status by appending one line:
   ...
   If an instruction from firstmate starts with \`[ack={token}]\` (it arrives as an inbox message like any other), it is a token-marked order:
   FIRST append \`resolved [key=ack-{token}] [at=<epoch>]: starting {one-line restatement of the order}\`
