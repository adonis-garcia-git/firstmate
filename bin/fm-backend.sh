...
# The sibling names arrive as separate arguments rather than one unquoted
# space-separated string, because zsh does not word-split unquoted expansions
# and this file must stay sourceable from an interactive zsh session.
fm_backend_siblings_readable() {  # <sibling-file>...
  local rel
  for rel in "$@"; do
...
fm_backend_source() {  # <name>
  ...
  case "$name" in
    tmux)
      fm_backend_siblings_readable fm-tmux-lib.sh fm-composer-lib.sh fm-cursor-lib.sh fm-session-lock-lib.sh fm-agent-process-lib.sh fm-gemini-lib.sh || return 1
      ;;
    herdr)
      fm_backend_siblings_readable fm-composer-lib.sh fm-transition-lib.sh fm-agent-process-lib.sh fm-session-lock-lib.sh fm-gemini-lib.sh || return 1
      ;;
    zellij)
      fm_backend_siblings_readable fm-backend-hometag-lib.sh fm-composer-lib.sh || return 1
      ;;
    orca)
      fm_backend_siblings_readable fm-composer-lib.sh || return 1
      ;;
    cmux)
      fm_backend_siblings_readable fm-backend-hometag-lib.sh fm-composer-lib.sh || return 1
      ;;
