...
#          pr_reconcile_sweep) while still printing every read-only detect line
...
pr_reconcile_sweep() {
  ...
  # It talks to GitHub, so it runs in the deferred network phase
  # (FM_BOOTSTRAP_NETWORK=only), never on the blocking local pass.
...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  if network_phase && network_sweep_authorized 'recorded-PR reconcile'; then
    __fm_timing_stamp=$(fm_timing_now_ms)
    pr_reconcile_sweep
