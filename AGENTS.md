...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, Relay artifact writes, and the recorded-PR reconcile sweep - run only when this session actually holds the lock from step 1; the five network ones among them run in the deferred stage rather than in this section.
...
For an order whose silent loss would leave supervision blind, send it with `fm-send`'s opt-in `--ack` flag so an unacknowledged order surfaces on its own within minutes; the script's help owns the mechanics.
...
3. For `check:`, act on the named poll result, including merges, recorded PRs found merged or closed externally, Relay events, process-to-event source results, and the once-daily `decision-digest` of waiting captain decisions, whose ranked top unblockers you relay to the captain as concise outcome-focused prose.
