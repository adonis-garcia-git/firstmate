...
Never rewrite a PR description without carrying that whole section through verbatim: each check run first judges the body carried by its triggering event, so a body edit that drops the section fails that `edited` event and leaves every later `synchronize` event failing too.
A failed event verdict falls back to briefly waiting for the live body's attestation to bind the event's head and re-judging the live body with the same pinned action (`bin/fm-attestation-rebind-wait.sh` feeds it), so restore the section binding the current head and re-run the latest failed check job to clear it.
