...
Restoring the exact section clears the check only while its attestation still binds the current head - once the head has moved, e.g. after the pipeline's own CI-fix commits, the restored body keeps failing because the attestation is bound to a previous head.
