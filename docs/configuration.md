...
That name is seeded from the hostname the first time it is needed and then kept in `state/.dispatch-machine`, so a machine that is later renamed still recognizes the claims it posted under its old name instead of going quiet about them.
The claim comments arrive with the label listing the poll already makes, so telling one machine's claim from the other's costs no extra request and one busy repository cannot use up the sweep budget that every repository after it needs.
...
That closes the only supported route to a `fm:building` issue with no claim comment, which the other machine would otherwise read as unclaimed and build a second time.
