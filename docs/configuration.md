...
The branch's runtime files have these format and lifecycle owners: `bin/fm-branch-outcome.sh` for the durable outcome store, its cursors, and the per-task status-coverage caches; [`pi-supervision-branch.md`](pi-supervision-branch.md) for the branch conversations and dialog-mirror cursor; [`watcher-continuity.md`](watcher-continuity.md) for the per-actor wake-row claims; and `bin/fm-lease-lib.sh` for the per-task supervision leases.
...
Each run's test artifacts therefore stay on local disk under the no-mistakes home instead of publishing to the orphan `no-mistakes/evidence` branch; the tracked file's own comments record the rationale for declining that upstream default.

Evidence never enters a pushed feature branch or the default branch either way: the worktree's `.no-mistakes/` stays local and CI rejects tracked entries under that path.
...
FM_SYSTEM_WAKE_EPOCH_OVERRIDE=   # pin the last-system-wake epoch used by awake-time beacon ages in fm-wake-lib.sh (0 means the system never slept), mainly for tests and diagnostics
...
FM_DECISION_DIGEST_INTERVAL=86400   # seconds between decision-wait digest wakes (bin/fm-decision-wait.sh); nothing waiting means no wake and a re-check on the next sweep
FM_COMPLETION_SCAN_INTERVAL=45   # seconds between completion-alarm reconciliation sweeps; paced on its own restart-surviving marker because each sweep may run one bounded current-state read per task
FM_COMPLETION_ALARM_WINDOW_SECS=90   # seconds an unhandled terminal state must persist before its one actionable wake; bin/fm-completion-alarm-lib.sh owns the full contract
FM_COMPLETION_SCAN_BUDGET_SECS=30   # wall-clock seconds one sweep may spend before deferring its remaining tasks to the next sweep, which resumes where it stopped; 0 disables the bound
FM_COMPLETION_READ_TIMEOUT_SECS=20   # seconds allowed for one reconciliation read, so a wedged reader cannot stall the watcher poll; 0 disables the bound
FM_PR_RECONCILE_INTERVAL=3600   # seconds between recorded-PR reconcile sweeps; bin/fm-pr-reconcile.sh owns the full contract
FM_PR_RECONCILE_MAX=30   # recorded PR URLs queried per reconcile sweep; a record set above the cap rotates across sweeps
FM_PR_RECONCILE_TIMEOUT=20   # seconds allowed for the reconcile sweep's single batched GitHub query
...
FM_GUARD_GRACE=300      # beacon freshness threshold in seconds; the awake-time clamp (time the system spent asleep never counts) applies where readers use fm_path_age_since_system_wake - guard verdicts, arm health checks, and the primary turn-end guard's watcher-health check - while the guard's away-mode daemon branch in bin/fm-turnend-guard.sh deliberately compares raw wall-clock age; see docs/turnend-guard.md for model-aware exceptions
...
FM_WATCHER_STALE_GRACE=300   # defaults to FM_GUARD_GRACE if set, else the poll-derived grace (docs/turnend-guard.md "Guard grace and the poll cadence"); seconds of awake time a live watcher lock may have a stale beacon before re-arm errors
FM_WATCHER_STALL_BOUND=       # defaults to 3x FM_WATCHER_STALE_GRACE; a live holder whose beacon is stale past this hard bound of awake time is evicted with TERM and replaced by the re-arm rather than refused (docs/turnend-guard.md, bin/fm-watch.sh header)
