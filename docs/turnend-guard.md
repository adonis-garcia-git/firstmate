...
Beacon staleness counts awake time only, so a beacon predating a system sleep window does not block after the wake (`fm_path_age_since_system_wake` in `bin/fm-wake-lib.sh`).
...
When a fresh beacon's singleton lock still names a dead holder pid, the beacon outlived its writer; the shared status computation reports it not-fresh and the banner names that dead holder (`fm_sup_watcher_lock_pid_dead` in `bin/fm-supervision-lib.sh`).
