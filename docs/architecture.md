...
The same restart-surviving check cadence also paces captain-decision latency tracking: each sweep reconciles `bin/fm-decision-wait.sh`'s durable first-observed record of captain-kind backlog holds and open needs-decision status waits, and at most once per `FM_DECISION_DIGEST_INTERVAL` (one day by default) a non-empty ranked digest of the top unblockers is queued as a `check: decision-digest:` wake for the supervising session to relay.
No waiting decisions means no wake, and the script header owns the record format, the ranking (unblocking power first, then age), and the read commands.
A separate bounded reconcile sweep, `bin/fm-pr-reconcile.sh`, periodically compares every recorded PR - `pr=` task metadata plus GitHub PR URLs referenced by open backlog items - against the forge in one batched query, and queues a durable check wake when a recorded PR was merged or closed externally, so parked records and captain-gated holds cannot keep re-surfacing a decision the forge already settled.
The sweep detects and surfaces only, never rewriting metadata or backlog items; the watcher runs it on the slow check cadence, locked session start runs it once in bootstrap's deferred network stage, and the script's header owns its throttle, bounds, caching, GitHub-only scope, and offline one-diagnostic contract.
...
On its own bounded cadence the watcher also reconciles every live non-secondmate task's current state through `bin/fm-crew-state.sh` and escalates a completion whose terminal state persisted unhandled past a bounded window exactly once per episode, through durable `state/pending-completions/` records owned by `bin/fm-completion-alarm-lib.sh`, so a finished-and-parked worker whose completion wake was lost or whose watcher had lapsed still surfaces within minutes of the first healthy cycle.
...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the fetched tip of the project's current default branch, and any unsafe or unverifiable base stops the spawn.
That tip is resolved from origin, or from the primary local copy's default-branch ref when a local-only project's pooled worktree has no `origin` remote.
