...
The sweep detects and surfaces only, never rewriting metadata or backlog items; the watcher runs it on the slow check cadence, the locked session-start bootstrap runs it once before the wake-queue drain, and the script's header owns its throttle, bounds, caching, GitHub-only scope, and offline one-diagnostic contract.
...
On `attached` it stays live across identity-matched successors, and an unexplained clean child close either attaches to a verified healthy successor or becomes a typed nonzero `watcher: FAILED` result; [`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the typed failure reasons.
