...
`tests/fm-dispatch-pickup.test.sh` did not exist on either run, so its 7784 ms hint is a single local measurement taken when the script was added on 2026-08-27 and should be replaced by a CI artifact figure at the next refresh.
