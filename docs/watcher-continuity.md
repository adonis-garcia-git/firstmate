...
- Classified reason.
- Beacon age (deliberately wall-clock, so rows can be correlated with system sleep windows).
...
- Awake-time beacon judgment across a simulated system sleep window.
- The typed live-holder hang failure.
- The dead-holder false-beacon alarm: a fresh beacon over a dead lock pid raises WATCHER DOWN naming that pid, while a live holder or a released lock stays silent.
...
- Exit-2 translation.
- Silent identity-walk classification of a dash-leading login-shell comm.
...
It also covers generation-claim single-flight, stuck-claim supersession, superseded-owner silence, notice-marker refusal and retry, ownership-atomic episode reset, and the legacy upgrade shim that reclaims a proven-abandoned pre-generation claim.
