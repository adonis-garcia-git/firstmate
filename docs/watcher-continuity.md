...
When the failed health check still shows a live identity-matched lock holder, the typed failure instead names that pid and its frozen beacon as a possible hang to inspect.
...
A fresh beacon is trusted only while the watcher singleton lock names a live pid: the watcher touches the beacon at the top of a cycle and can die later in the same cycle, so `fm_supervision_status` in `bin/fm-supervision-lib.sh` reports the watcher not-fresh when the recorded lock holder is dead.
The WATCHER DOWN alarm then fires instead of letting a beacon that outlived its writer mask the death for the rest of the grace window, and its printed evidence names the dead lock holder rather than a bare fresh-looking age.
A released or absent lock stays grace-tolerant, so the benign re-arm gap between a clean watcher exit and the next cycle still cannot false-alarm mid-turn.
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, awake-time beacon judgment across a simulated system sleep window, the typed live-holder hang failure, and the dead-holder false-beacon alarm (a fresh beacon over a dead lock pid raises WATCHER DOWN naming that pid, while a live holder or a released lock stays silent).
`tests/fm-subagent-pretool-check.test.sh` proves Claude retains only the non-status Bash seatbelts.
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, exit-2 translation, and silent identity-walk classification of a dash-leading login-shell comm.
It also covers generation-claim single-flight, stuck-claim supersession, superseded-owner silence, notice-marker refusal and retry, ownership-atomic episode reset, and the legacy upgrade shim that reclaims a proven-abandoned pre-generation claim; [`turnend-guard.md`](turnend-guard.md) owns those behavior contracts.
