...
# starts the worker from the fetched origin tip or stops when a configured
# origin is unusable.
# A local-only project has no origin configuration at all, so its pooled
# worktree freshens from the primary local copy instead; the same cleanliness,
# verification, and refusal guarantees are asserted on that path.
set -u
...
test_originless_pool_at_primary_tip_launches() {
  ...
  assert_not_contains "$out" "could not fetch origin" \
    "spawn attempted an origin fetch against a pool with no origin configuration"
  [ "$(git -C "$POOL_DIR" rev-parse HEAD)" = "$before" ] \
    || fail "spawn moved HEAD on an origin-less pool already at the primary default-branch tip"
  if [ "${FM_TEST_EVIDENCE:-0}" = 1 ]; then
    printf '# observed origin-less launch: %s\n' "$(printf '%s\n' "$out" | tail -n 1)"
  fi
  pass "an origin-less pool already at the primary tip launches through the primary-copy freshen"
}
